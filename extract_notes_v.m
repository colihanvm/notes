% verbose version for checkoff 3
...
function [notes] = extract_notes_v(im, threshold, num_find, radius)
...
figure; imshow(im); title('pixel color adjust'); pause;
%% 
...
figure; plot(tmp);  title('row sum'); pause;
figure; imshow(im); title('lines erased'); pause;
%% 
...
x=1:size(im,2);
plot(x, staffs*ones(size(x))); pause;
%%
...
figure; imshow(im); title('gap stitching complete'); pause;
%%
...
for stanza=1:num_stanza
    ...
    edges = detectEdges_removeLines_v(blank, threshold, 1); %verbose version
    chunk_centers = detectCircles(blank, edges, radius, num_find, acc);
   
    figure; imshow(blank); title('all circles found');
    viscircles(chunk_centers, radius * ones(size(chunk_centers, 1), 1)); hold on; pause
    ...
    figure; imshow(blank); title('post processed circles');
    viscircles(chunk_centers, radius * ones(size(chunk_centers, 1), 1)); hold on; pause
...
plot(x, staffs*ones(size(x))); title('finished result');
